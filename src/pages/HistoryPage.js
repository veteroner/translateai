...
const HistoryPage = () => {
  const { translations, loading, toggleFavorite, deleteTranslation } = useTranslationHistory();
  ...
  if (loading) {
    return (
      <PageContainer>
        <PageTitle theme={theme}>Çeviri Geçmişi</PageTitle>
  ...
  return (
    <PageContainer>
      ...
      {filteredTranslations.length === 0 ? (
        <EmptyState theme={theme}>
          <p>Henüz çeviri geçmişiniz bulunmuyor.</p>
        </EmptyState>
      ) : (
        <TranslationList>
          {filteredTranslations.map((translation) => (
            <TranslationCard key={translation.id} theme={theme}>
              ...
              <CardActions>
                <IconButton 
                  icon="favorite"
                  onClick={() => toggleFavorite(translation.id, !translation.isFavorite)}
                  title={translation.isFavorite ? "Favorilerden Çıkar" : "Favorilere Ekle"}
                  theme={theme}
                  isActive={translation.isFavorite}
                />
                <IconButton 
                  icon="copy"
